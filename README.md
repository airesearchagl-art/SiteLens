...
SiteLens は、動画・写真を報告書化しやすい形へ整理し、PowerPoint報告書まで生成する建築現場メディア整理ツールです。
...
出力先の初期候補は、元動画と同じフォルダ内の `SiteLens_Report` です。

例：

```text
D:/SiteMovie/site-video.mp4
D:/SiteMovie/SiteLens_Report/site-video_0001.jpg
```
...
このJSONは、レビュー情報の再利用や報告書生成の元データとして利用します。

## PowerPoint報告書生成

選択されている画像のみを対象に、以下のファイル名でPowerPoint報告書を出力します。

```text
ReviewReport_YYYYMMDD.pptx
```

スライド構成：

- 1枚目：表紙（工事名、撮影日、撮影者、場所）
- 2枚目以降：1画像1スライド
- 左側：画像
- 右側：タイムコード、優先度、タグ、階、通り芯、部屋名、コメント
